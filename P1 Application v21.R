...
#######################################################
# loading the data
dat <- read_xlsx("G:/My Drive/VTech/PhD Research/P1/data.xlsx")
...
cnt_tbl = as.data.frame(table(unlist(dat[(dim(dat)[1]),]))<=4)
cnt_tbl = cbind(cnt_tbl, rownames(cnt_tbl))
cnt_tbl = cnt_tbl[1:(dim(cnt_tbl)[1]-1),]
colnames(cnt_tbl) = c('status', 'pwy_number')
cnt_tbl = cnt_tbl[cnt_tbl$status==TRUE,]

# deleting small gene pathways
deletr = numeric()
listr = list(as.double(cnt_tbl$pwy_number))[[1]]
j=0
for(i in 1:(dim(dat_filt)[2])){
  if ((dat_filt[(dim(dat_filt)[1]),i] %in% listr)){
    j = j+1
    deletr[j] = i
  }
}
dat_filt = dat_filt[-deletr]
dat_og = dat
dat = dat_filt
dat = dat[,c(1,3538:(dim(dat)[2]))]
dat = dat[,1:6614]

pwys = unique(unlist(dat[(dim(dat)[1]),]))
num_pwy=length(pwys)-1 # Number of Pathways
...
#######################################################
pwy_s = num_pwy
n_s = dim(dat)[1]-1 # Number of observations

for(pwy_s in c(num_pwy)){
  for(n_s in c(dim(dat)[1]-1)){ # Number of observations)){
    #######################################################
    ##########     Simulation Settings     ################
    #######################################################
    ...
    sim_iters = 1 # Number of simulation iterations 
    skipper = 100
    modar = 1
    ...
    for(s in 1:sim_iters){
      ...
      # Creating a y 
      #y =  10*cos(first_path_features$X1)+3*(first_path_features$X2^2)+first_path_features$X1*first_path_features$X2*first_path_features$X4-sin(second_path_features$X3)*(second_path_features$X2)/4+2/5*(second_path_features$X3)*(second_path_features$X2)*(second_path_features$X4)+rnorm(nx)#+rALD(nx) 
      ...
      # Creating a y 
      y = dat$`Glucose 120 capillary blood who 85` # data
      
      og_df = dat[-c((length(y))),]
      group_identifier_vector = unlist(dat[(length(y)),])
      group_list = unique(as.numeric(group_identifier_vector[-1]))
        ...
        for (i in 2:N){
          ...
          if(distn == "mvn"){
            ...
            for (j in 1:num_pw){
              ...
              Ld = sum(log(dnorm((y-ald_theta*ald_z_vec)/(ald_tau^2), Reduce("+",(Map('%*%',kmat_dfs_tstr,alpha_mats_k)))/(ald_tau^2),ald_z_vec)),na.rm=TRUE)+ln(prior_ising_d)
              
              Le = sum(log(dnorm((y-ald_theta*ald_z_vec)/(ald_tau^2), (Reduce("+",(Map('%*%',kmat_dfs_tstr,alpha_mats_k)))-kmat_dfs[[j]]%*%alpha_mats[[j]][i,])/(ald_tau^2),ald_z_vec)),na.rm=TRUE)+ln(prior_ising_e)
      ...
      string1s = paste("G:/My Drive/VTech/PhD Research/P1/Real_Appl/RAP_MLN_G_mat", "_P", as.character(num_pwy),"_N", as.character(nx), "_S", as.character(s),".csv", sep = "")
      string2s = paste("G:/My Drive/VTech/PhD Research/P1/Real_Appl/RAP_MLN_mat", "_P", as.character(num_pwy),"_N", as.character(nx), "_S", as.character(s),".csv", sep = "")
      string3s = paste("G:/My Drive/VTech/PhD Research/P1/Real_Appl/RAP_Lasso_mat", "_P", as.character(num_pwy),"_N", as.character(nx), "_S", as.character(s),".csv", sep = "")
      string4s = paste("G:/My Drive/VTech/PhD Research/P1/Real_Appl/RAP_GLasso_mat", "_P", as.character(num_pwy),"_N", as.character(nx), "_S", as.character(s),".csv", sep = "")
      string5s = paste("G:/My Drive/VTech/PhD Research/P1/Real_Appl/RAP_SGL_mat", "_P", as.character(num_pwy),"_N", as.character(nx), "_S", as.character(s),".csv", sep = "")
      string6s = paste("G:/My Drive/VTech/PhD Research/P1/Real_Appl/RAP_SSGL_mat", "_P", as.character(num_pwy),"_N", as.character(nx), "_S", as.character(s),".csv", sep = "")
      ...
      write.csv(colnames(dat[,2:dim(dat)[2]]), "G:/My Drive/VTech/PhD Research/P1/Real_Appl/actual_gene_names.csv", row.names = FALSE)
      write.csv(group, "G:/My Drive/VTech/PhD Research/P1/Real_Appl/actual_gene_groups.csv", row.names = FALSE)
    }
    
    string1 = paste("G:/My Drive/VTech/PhD Research/P1/Real_Appl/RAP_MLN_G_mat", "_P", as.character(num_pwy),"_N", as.character(nx),".csv", sep = "")
    string2 = paste("G:/My Drive/VTech/PhD Research/P1/Real_Appl/RAP_MLN_mat", "_P", as.character(num_pwy),"_N", as.character(nx),".csv", sep = "")
    string3 = paste("G:/My Drive/VTech/PhD Research/P1/Real_Appl/RAP_Lasso_mat", "_P", as.character(num_pwy),"_N", as.character(nx),".csv", sep = "")
    string4 = paste("G:/My Drive/VTech/PhD Research/P1/Real_Appl/RAP_GLasso_mat", "_P", as.character(num_pwy),"_N", as.character(nx),".csv", sep = "")
    string5 = paste("G:/My Drive/VTech/PhD Research/P1/Real_Appl/RAP_SGL_mat", "_P", as.character(num_pwy),"_N", as.character(nx),".csv", sep = "")
    string6 = paste("G:/My Drive/VTech/PhD Research/P1/Real_Appl/RAP_SSGL_mat", "_P", as.character(num_pwy),"_N", as.character(nx),".csv", sep = "")
    ...
    write.csv(MSE_ph_mat, "G:/My Drive/VTech/PhD Research/P1/Simulations_mod7/MSE_ph_mat.csv", row.names = FALSE)
  }
}
