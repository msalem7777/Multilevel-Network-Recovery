...
first_path_features = data.frame(matrix(rmvnorm(nx,rep(0,4),sigma), nrow = nx))
...
# Creating a y 
y =  10*cos(first_path_features$X1)+3*(first_path_features$X2^2)+first_path_features$X1*first_path_features$X2*first_path_features$X4-0.5*sin(second_path_features$X3)+rnorm(nx)#+rALD(nx) 
...
VB = function(y, X, gamma_status=1, corrmat, LB_mat, LB=0.3, h=0.5, lambda = 0.9, iters=50, sigmasq=(0.5/2)^2, plot_on=FALSE, dist_true = FALSE){
  ...
  if(dist_true==FALSE){
    logodds = log(dnorm(y, mean = max(mus$centers), sd = max(mus$centers)/3)*as.numeric(y<1)/(2*dnorm(y, mean = 0, sd = max(mus$centers)/3)*as.numeric(y>0)))
  } else if(dist_true==TRUE){
    logodds = log(dnorm(y, mean = max(mus$centers), sd = max(mus$centers)/3)*as.numeric(y<1)/(df(y, df1 = 1, df2 = length(y)-2)))
  }
  ...
  logm1 = log(2*dnorm(y, mean = 0, sd = sqrt(sigmasq))*as.numeric(y>0))
  #j=1
  for (i in 1:iters){
    if(i>15){
...
h = matrix(0.5, nrow = N, ncol = num_pw)
...
for (i in 2:N){
  ...
  if(distn == "mvn"){
    for (j in 1:num_pw){
      ...
      alpha_mats[[j]][i,] = rmvnorm(1, solve(sigmasq[i-1,]/sigmasq_alpha[i-1,]*kmat_dfs[[j]]+t(kmat_dfs[[j]])%*%kmat_dfs[[j]])%*%t(kmat_dfs[[j]])%*%(y-0.5*Reduce("+",(Map('%*%',kmat_dfs_gammad,alpha_mats_k)))+0.5*kmat_dfs[[j]]%*%alpha_mats[[j]][i-1,]),sigmasq[i-1,]*solve(sigmasq[i-1,]/sigmasq_alpha[i-1,]*kmat_dfs[[j]]+t(kmat_dfs[[j]])%*%kmat_dfs[[j]]))
    }
  ...
  if(distn == "mvn"){
    for (j in 1:num_pw){
      ...
      d = dmvnorm(y, (Reduce("+",(Map('%*%',kmat_dfs_tstr,alpha_mats_k)))),sigmasq[i,]*diag(length(y)))#*prior_ising_H0
      
      e = dmvnorm(y, (Reduce("+",(Map('%*%',kmat_dfs_tstr,alpha_mats_k)))-kmat_dfs[[j]]%*%alpha_mats[[j]][i,]),sigmasq[i,]*diag(length(y)))#*(1-prior_ising_H0)
    ...
    for (j in 1:num_pw){
      ...
      Ld = sum(log(dnorm((y-ald_theta*ald_z_vec)/(ald_tau^2), Reduce("+",(Map('%*%',kmat_dfs_tstr,alpha_mats_k)))/(ald_tau^2),ald_z_vec)),na.rm=TRUE)#*prior_ising_H0
      
      Le = sum(log(dnorm((y-ald_theta*ald_z_vec)/(ald_tau^2), (Reduce("+",(Map('%*%',kmat_dfs_tstr,alpha_mats_k)))-kmat_dfs[[j]]%*%alpha_mats[[j]][i,])/(ald_tau^2),ald_z_vec)),na.rm=TRUE)#*(1-prior_ising_H0)
      
      
      d = prod(dnorm((y-ald_theta*ald_z_vec)/(ald_tau^2), Reduce("+",(Map('%*%',kmat_dfs_tstr,alpha_mats_k)))/(ald_tau^2),ald_z_vec))#*prior_ising_H0
      
      e = prod(dnorm((y-ald_theta*ald_z_vec)/(ald_tau^2), (Reduce("+",(Map('%*%',kmat_dfs_tstr,alpha_mats_k)))-kmat_dfs[[j]]%*%alpha_mats[[j]][i,])/(ald_tau^2),ald_z_vec))#*(1-prior_ising_H0)
  ...
  if(mthd == 'MCMC'){
    ...
    if((i==2)|(i%%100==0)){
        for (j in 1:num_pw){
          ...
          curr_xi_dfs[[j]][i,] = round(VB(y,pwy_dfs[[j]], corrmat = corr_mats[[j]], LB_mat = corr_LBs[[j]], gamma_status = -1*(-1^gamma[i,j]), h=h[i,j], lambda = 0.7, iters = 15, dist_true = FALSE)>0,0)
...
kmat_fin = Map('*',kmat_dfs,(colMeans(gamma)>=0.5)*1)

print(paste("Correlation between full model and the response variable is:", cor(Reduce("+",(Map('%*%',kmat_fin,alpha_mats_k))),y))) 
...
viz_net_obj = ggnetwork(net, layout = "kamadakawai")
viz_net_obj_mod = viz_net_obj
viz_net_obj_mod$x = viz_net_obj_mod$x+num_pw/2*(cos((as.numeric(viz_net_obj_mod$pathway)*pi/(num_pw))))
viz_net_obj_mod$y = viz_net_obj_mod$y+num_pw/2*(sin((as.numeric(viz_net_obj_mod$pathway)*pi/(num_pw))))
viz_net_obj_mod$xend = viz_net_obj_mod$xend+num_pw/2*(cos((as.numeric(viz_net_obj_mod$pathway)*pi/(num_pw))))
viz_net_obj_mod$yend = viz_net_obj_mod$yend+num_pw/2*(sin((as.numeric(viz_net_obj_mod$pathway)*pi/(num_pw))))
...
centroid_df = viz_net_obj_mod_rest %>%
  ...
  dplyr::summarise(
    ...
    yend = max(y,na.rm=TRUE),
    yrange = max(y,na.rm=TRUE)-min(y,na.rm=TRUE),
    y = (max(y,na.rm=TRUE)+min(y,na.rm=TRUE))/2
  )
  
radiuss = apply(centroid_df[,c(4,7)], 1, max)/2

viz_net_obj_mod_rest = viz_net_obj_mod_rest[order(viz_net_obj_mod_rest$pathway),]

gg <- ggplot(viz_net_obj_mod_rest, 
       aes(x, y, xend = xend,yend = yend)) +
  geom_point() +
  geom_point(data = centroid_df,  shape = 21, col= "black", size=radiuss*101, stroke = 1.0)+
  ...
  xlim((min(viz_net_obj_mod_rest$x)-0.25), (max(viz_net_obj_mod_rest$x)+0.25))+
  ylim(min(viz_net_obj_mod_rest$y)-0.25, max(viz_net_obj_mod_rest$y)+0.25)

cols = unique(c(ggplot_build(gg)$data[[4]]$colour,ggplot_build(gg)$data[[3]]$colour))

ggplot(viz_net_obj_mod_rest, 
       aes(x, y, xend = xend,yend = yend)) +
  geom_point() +
  geom_point(data = centroid_df,  shape = 21, col= cols, size=radiuss*101, stroke = 1.0)+
  #annotate("point", x = 1, y = 1, shape = 21, size=50, colour = "blue", fill= NA)+
  geom_edges(aes(color = pathway), alpha = 0.25) +
  geom_nodes(aes(color = pathway), size = 1.5) +
  scale_color_brewer("Pathway", palette = "Set1")+
